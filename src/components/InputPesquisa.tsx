...
type InputPesquisaProps = {
  setJogos: React.Dispatch<React.SetStateAction<JogoI[]>>;
};
...
export function InputPesquisa({ setJogos }: InputPesquisaProps) {
  ...
  async function enviaPesquisa(data: Inputs) {
    ...
    try {
      ...
      if (dados.length === 0) {
        alert("Não há jogos com o termo informado... Realize nova pesquisa.");
        reset({ termo: "" });
        return;
      }
      setJogos(dados);
  ...
  async function mostraDestaques() {
    try {
      ...
      alert("Erro ao buscar jogos em destaque. Tente novamente.");
    }
  }
      ...
      <form className="flex-1" onSubmit={handleSubmit(enviaPesquisa)}>
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Qual jogo você procura? (título, categoria ou desenvolvedora)"
            required
            {...register("termo")}
          />
          <button
